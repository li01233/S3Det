...
class Compose:
    ...
    def __call__(self, data, ratio_small):
        """Applies a series of transformations to input data."""
        for t in self.transforms:
            try:
                data = t(data)
            except:
                data = t(data, ratio_small)
        return data
...
class BaseMixTransform:
    ...
    def __init__(self, dataset,mosaic_small_mode = False,pre_transform=None, p=0.0) -> None:
        """Initializes the BaseMixTransform object with dataset, pre_transform, and probability."""
        self.dataset = dataset
        self.pre_transform = pre_transform
        self.p = p
        self.mosaic_small_mode = mosaic_small_mode

    def __call__(self, labels, ratio_small):
        ...
        # mosaic small mode
        n = 4
        if self.mosaic_small_mode:
            if random.uniform(0, 1) > ratio_small:
                n = 9
        ...
        if self.pre_transform is not None:
            for i, data in enumerate(mix_labels):
                mix_labels[i] = self.pre_transform(data,ratio_small)
        labels['mix_labels'] = mix_labels

        # Feedback CutPaste
        if random.uniform(0, 1) > ratio_small:
                labels = self._cutpaste_transform(labels)

        # Mosaic or MixUp
        if self.mosaic_small_mode:
            labels = self._mix_transform_feedback(labels, n)
        else:
    ...
    def _mix_transform_feedback(self, labels, n):
        raise NotImplementedError
    
    def _cutpaste_transform(self,labels):
        raise NotImplementedError
...
class Mosaic(BaseMixTransform):
    ...
    def __init__(self, dataset, imgsz=640, p = 1.0, n = 4, small_mode=False):
        """Initializes the object with a dataset, image size, probability, and border."""
        assert 0 <= p <= 1.0, f'The probability should be in range [0, 1], but got {p}.'
        assert n in (4, 9), 'grid must be equal to 4 or 9.'
        super().__init__(dataset=dataset, p=p,mosaic_small_mode=small_mode)
        self.dataset = dataset
        self.imgsz = imgsz
    ...
    def _mix_transform_feedback(self, labels, n):
        ...
        ###mosaic feedback
        return self._mosaic3(labels) if n == 3 else self._mosaic4(labels) if n == 4 else self._mosaic9(
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    pre_transform = Compose([
        Mosaic(dataset, imgsz=imgsz, p=hyp.mosaic,small_mode = hyp.mosaic_small_mode),
        CopyPaste(p=hyp.copy_paste),
        Feedback_CopyPaste(dataset, p=hyp.copypaste_small,n=hyp.copypaste_sample,
                           num_small=hyp.copypaste_num_small,small_theshold=hyp.copypaste_small_theshold),
        RandomPerspective(
            degrees=hyp.degrees,
...
####################################################################################################################################
class Feedback_CopyPaste(BaseMixTransform):

    def __init__(self, dataset, p=0.0, n=50, num_small=2, small_theshold=1.0) -> None:
        super().__init__(dataset=dataset, p=p)
        assert 0 <= p <= 1.0, f'The probability should be in range [0, 1], but got {p}.'
        self.dataset = dataset
        self.n = n
        self.num_small = num_small
        self.small_theshold = small_theshold

    def get_indexes(self):
        """Return a list of random indexes from the dataset."""
        # select any images
        return [random.randint(0, len(self.dataset) - 1) for _ in range(self.n)]
    ...
    def _cutpaste_transform(self,labels):
        ...
        ori_instance = deepcopy(labels['instances'])
        boxes = ori_instance.bboxes
        # convert to xywh
        boxes = self._convert_xywh(boxes,ori_h,ori_w)

        for i in range(self.n):
            labels_patch =  labels['mix_labels'][i]
            
            paste_instance = deepcopy(labels_patch['instances'])

            for j in range(len(paste_instance.bboxes)):
                box = deepcopy(paste_instance.bboxes[j])
                img_paste = labels_patch['img']
                h,w = img_paste.shape[:2]
                # judge if this is a small object
                if(self._judge_small(box)):
                    
                    box = torch.as_tensor(box)
                    # cal iou
                    boxes = torch.as_tensor(boxes)
                    iou = -1 * bbox_iou(box,boxes,CIoU=True)
                    if(torch.ge(iou,-0.7).all()):

                        # num of cutpaste things cant be too many
                        if num_small > self.num_small:
                            return labels
                        
                        # Load image and paste roi
                        # get roi
                        box = self._convert_xyxy(box,h,w)
                        img_paste = img_paste[int(box[1]):int(box[3]),int(box[0]):int(box[2])]
                        
                        # get random h,w
                        paste_h = random.randint(0, ori_h)
                        paste_w = random.randint(0, ori_w)
                        # get paste box(xywh format)
                        x1 = paste_w
                        y1 = paste_h
                        x2 = min(box[2] - box[0] + paste_w, ori_w)
                        y2 = min(box[3] - box[1] + paste_h, ori_h)
                        paste_box = np.array([[x1,y1,x2,y2]])
                        
                        # paste it and concatenate all instances
                        p_h,p_w = img_paste.shape[:2]
                        labels['img'][int(y1):int(y2),int(x1):int(x2)] = img_paste[0:min(p_h,ori_h-paste_h),0:min(p_w,ori_w-paste_w)]
                        tmp_instances = Instances(bboxes=paste_box,
                                            segments=paste_instance.segments,
                                            keypoints=paste_instance.keypoints)
                        labels['cls'] = np.concatenate((labels['cls'], np.array([labels_patch['cls'][j]])), 0)
                        labels['instances'] = Instances.concatenate((labels['instances'], tmp_instances), axis=0)
                        num_small += 1
                        if self.small_theshold > 0.005:
                            self.small_theshold *= 0.99999
        return labels
    
    def _convert_xywh(self,boxes,img_h,img_w):
        x1 = boxes[:,0]
        y1 = boxes[:,1]
        x2 = boxes[:,2]
        y2 = boxes[:,3]
        boxes[:,0] = ((x1+x2)/2)/img_w
        boxes[:,1] = ((y1+y2)/2)/img_h
        boxes[:,2] = (x2-x1)/img_w
        boxes[:,3] = (y2-y1)/img_h
        return boxes
    
    def _convert_xyxy(self,boxes,img_h,img_w):
        x = boxes[0] * img_w
        y = boxes[1] * img_h
        w = boxes[2] * img_w
        h = boxes[3] * img_h
        boxes[0] = max(math.ceil(x - w/2),0)
        boxes[2] = min(math.ceil(x + w/2),img_w)
        boxes[1] = max(math.ceil(y - h/2),0)
        boxes[3] = min(math.ceil(y + h/2),img_h)
        return boxes

    def _judge_small(self,box):
        return (box[2] * box[3]) < self.small_theshold
######################################################################################################################################