...
class BaseTrainer:
    ...
    def _setup_train(self, world_size):
        ...
        # change it to slice mode
        self.train_loader = self.get_dataloader(self.trainset, batch_size=batch_size, rank=RANK, mode='train', slice_mode = self.args.train_slice_mode)
        if RANK in (-1, 0):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    self.loss, self.loss_items, self.ratio_small = self.model(batch,self.ratio_small)
                    self.train_loader.dataset.update_ratio_small(self.ratio_small)
                    if RANK != -1:
