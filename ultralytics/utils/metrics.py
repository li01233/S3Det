...
def calculate_ratio_small(pred_scores, target_scores,pred_bboxes,target_bboxes,weight,target_scores_sum,
                          imgsz,stride_tensor,small_theshold,mode = 'cls_mode'):
    if mode == 'box_mode':
        iou = bbox_iou(pred_bboxes, target_bboxes, xywh=False, CIoU=True)
        loss = ((1.0 - iou) * weight) / target_scores_sum
    elif mode == 'cls_mode':
        bce = nn.BCEWithLogitsLoss(reduction='none')
        loss = bce(pred_scores, target_scores.to(pred_scores.dtype))

    total_loss = loss.sum()
    small_loss = loss[judge_small(target_bboxes,imgsz,stride_tensor,small_theshold)].sum()
    return small_loss / total_loss

def judge_small(gt_bboxes,imgsz,stride_tensor,small_theshold):
    gt_bboxes_w = gt_bboxes[:,:,2] - gt_bboxes[:,:,0]
    gt_bboxes_h = gt_bboxes[:,:,3] - gt_bboxes[:,:,1]
    
    gt_areas = gt_bboxes_w * gt_bboxes_h
    area = imgsz[0] * imgsz[1] / (stride_tensor * stride_tensor)
    area = area.permute(1,0)
    return gt_areas / area < small_theshold
