...
def plot_labels(boxes, cls, names=(), save_dir=Path(''), on_plot=None):
    ...
    cls_n = [names[int(i)] for i in cls]
    box_ratio = boxes[:,2] / boxes[:,3]
    x1 = pd.DataFrame(boxes, columns=['x', 'y', 'width', 'height'])
    x2 = pd.DataFrame(cls_n, columns=['type'])
    x3 = pd.DataFrame(cls, columns=['Instances'])
    x4 = pd.DataFrame(box_ratio, columns=['Aspect ratio'])
    x = x1.join(x2.join(x3.join(x4)))
    ...
    # Matplotlib labels
    pal = sn.cubehelix_palette(6, rot=-0.1,gamma=1.5,light=0.7)
    sn.set_style("whitegrid")
    sn.histplot(data=x,x='Instances', bins=nc,hue='type',palette=pal,shrink=0.9,edgecolor=None,alpha=1)
    name = [names[i] for i in range(nc)]
    plt.xticks(ticks=range(0,nc,1),labels=name,rotation = 20)
    plt.legend().remove()
    plt.savefig(save_dir / 'classes_details.pdf', dpi=200,bbox_inches='tight')
    plt.close()

    sn.set_style("white")
    sn.jointplot(x, x='x', y='y', kind="hist",color='cornflowerblue')   
    plt.savefig(save_dir / 'x-y.pdf', dpi=200)
    plt.close()

    sn.set_style("white")
    sn.jointplot(x, x='width', y='height', kind="hist",color='cornflowerblue')
    plt.savefig(save_dir / 'w-h.pdf', dpi=200)
    plt.close()
    ...
    for cls, box in zip(cls[:500], boxes[:500]):
        ImageDraw.Draw(img).rectangle(box, width=2, outline=colors(cls))  # plot
    plt.imshow(img)

    fname = save_dir / 'box_detail.pdf'
    ...
    pal = sn.cubehelix_palette(6, rot=-0.1,gamma=1.5,light=0.7)
    g = sn.FacetGrid(x, row="type", hue="type", aspect=15, height=.5, palette=pal,xlim=[0,10])

    # Draw the densities in a few steps
    g.map(sn.kdeplot, "Aspect ratio",
        bw_adjust=.5, clip_on=False,
        fill=True, alpha=1, linewidth=1.5)
    g.map(sn.kdeplot, "Aspect ratio", clip_on=False, color="w", lw=2, bw_adjust=.5)
